...
import { fetchPage } from '../actions/page-actions.js';
    ...
    if (!!this.props.thisPage) {
      const panes = [
        ...
        },
        {
          ...
          render: () => (
            <TabNgramItemGrid
              unigrams={filterUnigrams(this.props.thisPage.body_text).slice(
                0,
                12
              )}
              bigrams={filterBigrams(this.props.thisPage.body_text).slice(
                0,
                12
              )}
            />
...
function mapStateToProps(state) {
  ...
  const thisPageInsights = state.insights.filter(
    (insight) => insight.page_id === thisPageId
  );
  const thisPageComments = state.comments.filter(
    (comm) => comm.page_id === thisPageId
  );
