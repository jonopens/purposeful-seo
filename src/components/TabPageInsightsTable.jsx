...
import TabInsightTableRow from './TabInsightTableRow.jsx';

const TabPageInsightsTable = (props) => {
  const rows = props.insights.map((row, idx) => (
    <TabInsightTableRow key={idx} {...row} />
  ));
  if (rows.length > 0) {
    return (
      <Tab.Pane>
              ...
              <Table celled>
                ...
                <Table.Body>
                  {rows.length > 0 ? (
                    ...
                    <Table.Row>
                      <Table.Cell>
                        No Insights available for the current page.
                      </Table.Cell>
                    </Table.Row>
                  )}
                </Table.Body>
